# SOC 与 SVMP 平台定制化联动方案 V1.1

> **状态**：已被 V2.0 通用集成方案取代，本文档保留作 SOC 专项参考与部侧字段对照。  
> **请优先阅读**：[漏洞管理平台对外集成能力设计方案-V2.0.md](./漏洞管理平台对外集成能力设计方案-V2.0.md)
...
在 SOC 与 SVMP 之间增加 **SOC-SVMP 接入层**，负责协议转换、任务映射、结果标准化、异步回调及部侧 `vulInfoLst` 结构组装。

```
SOC ──REST/Kafka──▶ 接入层 ──SVMP原生API──▶ SVMP
         ◀──回调/推送──              ◀──扫描结果──
```

| 模块 | 职责 |
|------|------|
| 任务管理 | `socTaskId` ↔ SVMP `taskId` 映射 |
...
**SOC 接入层约定**：对外 API 以 **`vulInfoID`** 为主键；SVMP `vulnDisposalId` 仅作内部映射。
...
**列表项必返**：`vulInfoID`, `vulID`, `vulInfoStat`, `lvRsn`, `vulName`, `vulLevel`, `orgVulId`, `vulNetAddr`, `vulPort`, `vulSvc`, `isAccess`, `transferTime`, `vulnDisposalId`（SVMP 映射）
...
接入层负责 SVMP 字段 → `vulInfoLst` 字段映射（见下表）。
...
依据《测试规范（2025）》检测要点，SOC-SVMP 联动应满足：
...
| 编号 | 检查项 | 通过标准 |
|------|--------|----------|
| T-08-1 | 核验闭环 | SVMP verify 后状态 **6** 或 **7** |
...
| T-15 | 真实性 | 上报 `vulInfoStat` 与 SOC/SVMP 实测一致 |
