...
| 项 | 说明 |
|----|------|
| **网络安全漏洞管理平台** | 漏洞发现、验证、处置、修复核验与结果管理的统一业务系统，以下简称**平台** |
| **Partner** | 第三方系统（SIEM、ITSM、资产平台、安全运营系统等） |
| **本文档** | **平台**向 **Partner** 提供的 **API 接口**说明（Base Path `/api/open/v1`） |
| **与扫描器** | **Partner不直连**扫描器；排查、复扫等由平台内部扫描执行层调度（见 §1.1） |

> 接入前请确认 `partnerId`、业务域名、Token 端点均属于同一套**网络安全漏洞管理平台**实例；具体环境以运营开通材料为准（§2.2）。

### 1.1 能力范围
<a id="11-能力范围"></a>

**第三方系统**通过**本平台 API** 可完成：
...
```text
┌─────────────────────┐         HTTPS REST          ┌────────────────────────────────────┐
│  第三方系统 Partner  │ ◀────────────────────────▶ │  网络安全漏洞管理平台                 │
│  Partner 系统          │         Webhook POST         │  API  /api/open/v1                 │
└─────────────────────┘ ◀────────────────────────── └────────────────────────────────────┘
```

### 1.3 漏洞实例生命周期（写接口）
<a id="13-漏洞实例生命周期写接口"></a>

本节说明**会改变业务状态**的写操作数量与顺序；完整路径清单见 [§5.0](#50-接口一览)。
...
| 分类 | 业务步骤 | HTTP 方法 · 路径 | 数量 | 处置阶段 | 说明 |
|------|----------|------------------|:----:|------|------|
| **任务写** | ① 创建扫描任务 | `POST /tasks/file` | 1 | 排查 | 基于模板文件创建排查任务，产生 `taskId` 与实例，新产生的漏洞实例视为**1-初始发现**状态 |
|  |  | `POST /tasks/vul` | 1 | 排查 | 基于请求体创建排查任务，产生 `taskId` 与实例，新产生的漏洞实例视为**1-初始发现**状态 |
| **实例写** | ① 验证 | `POST /instances/{vulInfoID}/verify` | 1 | 验证 | 当Partner识别漏洞实例状态为**误报**时，可选择调用此接口变更漏洞实例状态为3-已验证误报，此状态为**终态**。 |
| | | `POST /instances/verify:batch` | 1 | 验证 | 批量，与单条语义相同 |
| | ② 处置 | `POST /instances/{vulInfoID}/remediate` | 1 | 修复 | 含修复成功与修复失败/备案 |
| | | `POST /instances/remediate:batch` | 1 | 修复 | 批量，与单条语义相同 |
| | ③ 修复核验 | `POST /instances/{vulInfoID}/verify-fix` | 1 | 核验 | 异步复扫；平台内部默认选用该漏洞实例**最近一次排查/扫描使用的扫描器**执行复扫，完成后更新 `vulInfoStat` 终态；Partner 可 Webhook 或轮询 |
| | | `POST /instances/verify-fix:batch` | 1 | 核验 | 批量；复扫与扫描器选择规则同单条 |
...
| 回调地址         | **Partner** 提供默认 `callbackUrl`（亦可在创建扫描任务时覆盖） |
| IP 白名单 / mTLS | 按安全要求可选                                               |
...
实际**域名、认证服务 Base URL**以平台运营开通材料为准。**注：获取 Token 不在 `/api/open/v1` 路径下。**
...
<a id="31-bearer-token调用业务-api"></a>

**v1.0.4 唯一支持的 REST 鉴权方式。** 业务接口（`/api/open/v1/*`）请求头：
...
<a id="311-获取-access-token"></a>

Token 由**平台认证/登录服务**签发（OAuth 2.0 Client Credentials），**不属于**本平台业务 API Base Path `/api/open/v1`。

| 项 | 说明 |
|----|------|
| 调用方 | Partner 服务端（勿在前端暴露 `clientSecret`） |
| 凭证来源 | 运营开通时分配的 `clientId`、`clientSecret`（与 `partnerId` 绑定） |
| 端点（二选一，以开通材料为准） | 标准：`POST {authBaseUrl}/oauth/token`； |
| `{authBaseUrl}` | **认证服务 Base URL**，见 [§2.2](#22-环境地址) 开通材料 |
...
<a id="32-方式二api-key-签名暂未开放"></a>

`X-Api-Key` + `X-Signature`（HMAC-SHA256）鉴权**不在 v1.0.4 范围内**；调用 REST API 须使用 §3.1 Bearer Token。该方式计划于后续版本提供。

### 3.3 Webhook 验签（与 REST 无关）
<a id="33-webhook-验签与-rest-无关"></a>

**Partner 调用本平台 REST API 时不需要 Webhook 验签，平台也不会因每次 REST 调用而回调 Partner。**
...
<a id="实例写操作verify-remediate-verify-fix"></a>
...
<a id="首次验证-vi-001clientrequestidreq-001"></a>
Idempotency-Key: verify:VI-20260517-0001:req-001

# 网络超时后重试（同一 req-001）→ 返回首次结果
<a id="网络超时后重试同一-req-001-返回首次结果"></a>
Idempotency-Key: verify:VI-20260517-0001:req-001

# 核验未修复后再次处置（新意图，须新 ID）
<a id="核验未修复后再次处置新意图须新-id"></a>
...
<a id="511-post-tasksfile-创建扫描任务xml-配置"></a>
...
<a id="512-post-tasksvul-创建扫描任务json"></a>
...
<a id="513-get-taskstaskid-查询任务进度"></a>
...
<a id="521-post-instancessearch-分页搜索"></a>
...
<a id="522-get-instancesvulinfoid-实例详情"></a>
...
<a id="531-post-instancesvulinfoidverify-单条验证"></a>
...
<a id="532-post-instancesverifybatch-批量验证"></a>
...
**工单字段（必填）**：对应部侧表56「系统漏洞修复类型日志」。`srcTktRole`、`dstTktRole` 见 [附录 C](#附录-c-平台用户角色-srctktrole-dsttktrole)；`assignerDept`、`handlerDept` **必填**；派单人 `assignerEmail` / `assignerPhone`、处置人 `handlerEmail` / `handlerPhone` **至少填一项**（部侧 `srcTktPrsn`/`dstTktPrsn` 为「部门,邮箱,电话」合并串，本平台 API **不提供**该合并字段）。

同一实例仅可成功处置一次；重复调用返回 **40005**。

#### 5.4.1 `POST /instances/{vulInfoID}/remediate` — 单条处置
<a id="541-post-instancesvulinfoidremediate-单条处置"></a>
...
<a id="542-post-instancesremediatebatch-批量处置"></a>
...
<a id="551-post-instancesvulinfoidverify-fix-单条"></a>

| 项   | 值                                                           |
| ---- | ------------------------------------------------------------ |
| 能力 | `INSTANCE_VERIFY_FIX`                                        |
| 说明 | 前置 **`vulInfoStat = 5`**；平台触发修复核验复扫，完成后推进状态并可选外发 |
...
**平台行为**：受理后异步执行**复扫**；平台内部默认选用该漏洞实例**最近一次排查/扫描使用的扫描器**调度复扫任务。受理期间 `vulInfoStat` 暂保持 **5**；复扫完成后更新终态，并通过 Webhook `INSTANCE_VERIFY_FIX_COMPLETED` 通知 **6 / 7 / 10**，并可产生 `EXPORT_READY`（`exportStage=VERIFY_FIX_SCAN`）。外发须含 `targets[]`、`liveProbeResults[]`、`portScanResults[]`、`vulnerabilities[]`（§5.6.3）。
...
<a id="552-post-instancesverify-fixbatch-批量"></a>
...
扫描结果外发输出物由平台按当前接口领域模型重新组装，支持 **`xml`** 与 **`json`** 两种 `format`；根结构为 `<TaskExport>` / `taskExport`，不作为引擎原始 XML 透传。
...
| 项       | 约定                                                         |
| -------- | ------------------------------------------------------------ |
| 字段来源 | 以本平台 API 的任务、实例、处置、备案、修复核验字段为准；引擎原始字段仅进入 `evidence` / `appendices` 等扩展节点 |
...
<a id="564-输出参数export-task-summary"></a>
...
<a id="565-输出参数targets-liveproberesults-portscanresults"></a>
...
<a id="566-输出参数vulnerabilities-产品漏洞聚合"></a>
...
<a id="568-get-exportsexportid-外发元数据"></a>
...
<a id="569-get-exportsexportiddownload-下载文件"></a>
...
<a id="5610-get-taskstaskidexports-任务外发历史"></a>
...
| 类型 | 字段 | 说明 |
|------|------|------|
| string | `X-Webhook-Signature` | 事件 body 的 HMAC-SHA256（密钥为运营分配的 Webhook Secret） |
| string | `X-Webhook-Timestamp` | Unix 秒时间戳；与服务器时间差 **5 分钟内**有效 |
...
| 参数             | 类型   | 必填 | 说明                                                 |
| ---------------- | ------ | :--: | ---------------------------------------------------- |
| items            | array  |  ✓   | 本批次核验完成的实例列表                             |
| totalCount       | int    |  ✓   | 本批次实例总数                                       |
| verifyFixStatus  | enum   |  ✓   | `FINISHED` / `FAILED`                                |
| verifyFixJobId   | string |  ✓   | 核验任务 ID                                          |
| batchId          | string |  ○   | 批次标识（如有），对应核验请求的 `clientBatchId`     |
...
```json
{
  ...
  "payload": {
    "totalCount": 3,
    "batchId": "verify-fix-batch-20260517-001",
    "items": [
      {
      ...
      {
        "vulInfoID": "VI-20260517-0005",
        "vulInfoStat": 7,
        "previousVulInfoStat": 5
      },
      {
        "vulInfoID": "VI-20260517-0012",
        "vulInfoStat": 10,
        "previousVulInfoStat": 5
      }
    ]
...
<a id="附录-c-平台用户角色-srctktrole-dsttktrole"></a>
...
<a id="附录-d-漏洞管理处置方式-srcmethod"></a>

摘自部侧规范 **A.10 漏洞管理处置方式码表**。本平台 API 在创建扫描任务（§5.1.1 / §5.1.2，见 [附录 I](#附录-i-部侧排查扩展参数)）、验证（§5.3）、处置（§5.4）等接口中以 `srcMethod` 传递**处置代码**列取值。
...
<a id="h1-扫描模板-scantemplateid-内联扫描阶段"></a>
...
1. **能力码**是**平台**对REST API接口的能力分类，仅用于**平台**为第三方接入系统接入授权使用，**Partner**无需使用，第三方平台可以使用的REST API参考**5.0 接口一览**

### J-1 · 写接口数量与 §1.3 的关系
<a id="j-1-写接口数量与-13-的关系"></a>

**疑问**：漏洞实例生命周期（写接口），看不懂这个具体多少个操作？

**答复**：

1. **任务创建**（产生 `taskId` 与初始实例，不属于上述实例状态机写接口）为 **2 个路径二选一**：
   - 排查阶段(按需选择任务创建接口)
     - `POST /tasks/file`（XML 配置文件）
     - `POST /tasks/vul`（JSON 排查参数）
2. **实例写接口**（改变 `vulInfoStat` 的 REST 写操作）固定为 **3 类 × 2 路径 = 6 个**：
   ...
   - 核验：`POST /instances/{vulInfoID}/verify-fix`、`POST /instances/verify-fix:batch`
3. **处置业务流程** 接口使用流程参考 **1.3.1 状态流转（业务语义）**、**1.3.2 写接口数量对照**
4. 本**平台**依据**工信部**下发的**《基础电信企业网络安全漏洞管理平台建设指南、接口规范和测试规范》**建设，完整的漏洞生命周期处置阶段应包含：

| 业务阶段 | 系统漏洞目标状态               | 处置方式                                                     |
| -------- | ------------------------------ | ------------------------------------------------------------ |
| 排查     | 初始发现                       | 1020 指纹插件（远程版本扫描） 1021 POC插件 1022 漏洞扫描（混合） 1023 （人工）线下导入 1024 （人工）本地发现 1027 登录扫描 1028 连通性检测 1030 字典组合暴破 1031 规则猜测暴破 1032 配置文件分析 |
| 验证     | 已验证有效 已验证误报 验证失败 | 1026 交叉扫描验证 1040 EXP漏洞利用                           |
| 修复     | 已修复 修复失败                | 1050 补丁修复 1051 等效防护修复 1052 连通性阻断(离线使用） 1053 资产下线 |
| 核验     | 核验修复 核验未修复 核验失败   | 1030 字典组合暴破 1031 规则猜测暴破 1032 配置文件分析 1060 修复核验自适应扫描 1061 攻击模拟漏洞扫描 |

### J-2 · Bearer Token 获取端点
<a id="j-2-bearer-token-获取端点"></a>

**疑问**：Bearer Token,Token 由平台登录服务签发,这个token是要调哪个接口？
...
| 2 | 服务端调用 Token 端点（开通材料指定其一）：<br>• 标准：`POST {authBaseUrl}/oauth/token`（`grant_type=client_credentials`）<br> |
| 3 | 解析响应中的 `access_token` / `accessToken`，填入业务请求头 `Authorization: Bearer ...` |
| 4 | 调用本平台 API：`https://{业务域名}/api/open/v1/...` |

> OpenAPI 文件（`openapi/v1/openapi.yaml`）**仅覆盖**业务契约 `/api/open/v1`；Token 端点不在 OpenAPI 范围内，以本节及运营开通手册为准。

### J-3 · Webhook 验签适用范围
<a id="j-3-webhook-验签适用范围"></a>

**疑问**：Webhook 验签，是每次调用接口，漏管平台都会回调接口吗？
...
**疑问**：幂等 可选Idempotency-Key；与extTaskId二选一即可，下面5.1.1示例中两个都存在，一个在请求体一个在请求头？
...
**疑问**：还有一些表格，表头有些叫项-约定，有些叫路径-类型-必填-说明，有些叫字段/XMl路径-类型-说明，有些叫参数-类型-说明，看着好混乱？
...
| 版本      | 日期       | 说明                                                         |
| --------- | ---------- | ------------------------------------------------------------ |
| **1.0.7** | 2026-07-17 | 新增扫描窗口与超时暂停处理：允许扫描时间段由平台窗口策略裁决，创建任务**§5.1.2** 响应新增 `estimatedStartAt`（预计启动时间）/ `matchedScanWindow` / `windowSource`；**§5.1.3/§5.1.4** 新增 `PAUSED_WAITING_SCAN_WINDOW`、`PAUSED_NO_SCAN_WINDOW` 及暂停字段；新增 **§5.1.5** 追加扫描窗口、**§5.1.6** 任务暂停、**§5.1.7** 任务启动、**§5.1.8** `GET /scan-windows/policy` 查询可用扫描窗口；**§6.2/§6.7** 新增 `TASK_PAUSED`、**§6.2** 新增 `SCAN_WINDOW_CHANGED` Webhook；**§8** 新增 `TASK_WINDOW_WRITE`、`TASK_CONTROL`；**§11** 新增扫描窗口与超时暂停处理规则 |
...
| **1.0.0** | 2026-05-17 | 首版发布：Partner REST API、Webhook、扫描结果外发（XML/JSON）、能力码与业务错误码 |