...
| 项 | 说明 |
|----|------|
| **平台（唯一）** | **网络安全漏洞管理平台** — 漏洞发现、验证、处置、修复核验与结果管理的统一业务系统 |
| **本文档描述对象** | 上述平台向 **外部第三方（Partner）** 提供的 **API 集成能力**，不是另一个独立系统 |
| **与「漏扫平台」** | **不是**独立漏扫产品或扫描器厂商系统；扫描/排查仅为平台内置能力之一（见 §1.1） |

#### 1.0.2 「开放平台 API」如何理解

文档标题中的 **「开放平台 API」** 是产品/工程侧对 **「对外集成 API」** 的习惯简称，**不是**第二个平台名称。

| 说法 | 含义 | 是否独立平台 |
|------|------|:------------:|
| **网络安全漏洞管理平台** | 业务系统本体（控制台、工单、扫描调度、漏洞实例等） | — |
| **开放平台 API** / **对外集成 API** / **Partner API** | 该平台向 SIEM、ITSM、资产平台等第三方开放的 **HTTPS REST 接口能力** | **否** |
| Base Path **`/api/open/v1`** | 上述对外 API 的技术入口；路径中的 `open` 表示 **对外集成模块**，非另建「开放平台」实例 | **否** |

**关系（包含，而非并列）**：

```text
网络安全漏洞管理平台
├── 平台内部能力（控制台、扫描执行、工单、部侧对接等）
└── 对外集成 API（开放平台 API）  ←  本文档规范的对象
         REST  /api/open/v1
         Webhook  回调 Partner
```
...
| 场景 | 推荐表述 |
|------|----------|
| 商务/立项 | 「对接 **网络安全漏洞管理平台** 的 **对外 API**」 |
| 技术对接 | 「调用 **`/api/open/v1`** 下的 REST 接口」 |
| 引用本文档 | 「《网络安全漏洞管理平台 · 开放平台 API 接口规范》」 — 「开放平台 API」为文档简称，与「对外集成 API」同义 |

> 接入前请确认 `partnerId`、业务域名、Token 端点均属于**同一套网络安全漏洞管理平台**实例；具体环境以运营开通材料为准（§2.2）。

### 1.1 能力范围

第三方通过**本平台对外集成 API**（下文亦称 **「开放平台 API」**，见 [§1.0.2](#102-开放平台-api如何理解)）可完成：
...
│  Partner 系统          │         Webhook POST         │  对外集成 API（开放平台 API）       │
└─────────────────────┘ ◀────────────────────────── │  /api/open/v1                      │
                                                      └────────────────────────────────────┘
```
...
| # | 疑问摘要 | 答复要点 | 详见 |
|---|----------|----------|------|
| J-0 | 开放平台是漏扫平台吗？对接的是哪个系统？ | 只有一个平台：**网络安全漏洞管理平台**；本文档规范其 **对外集成 API**，不是独立漏扫产品 | [§1.0.1](#101-平台是什么) |
| J-6 | 「开放平台 API」是不是另一个平台？如何理解这个名词？ | **不是**。「开放平台 API」= 网络安全漏洞管理平台向 Partner 开放的 **API 能力简称**，与「对外集成 API」同义 | [§1.0.2](#102-开放平台-api如何理解) |
...
1. **只有一个业务平台**：**网络安全漏洞管理平台**（漏洞全生命周期管理，含扫描/排查、验证、处置、修复核验等）。
2. **本文档规范什么**：该平台向 Partner 开放的 **对外集成 API**（REST `/api/open/v1` + Webhook），**不是**另建一套「开放平台」或「漏扫平台」系统。
3. **与漏洞扫描**：扫描由平台内部执行层调度；Partner 通过 API 编排任务与管理实例，**不直连**扫描器（§1.1）。

### J-6 · 「开放平台 API」名词如何理解

**疑问**：我们的平台是网络安全漏洞管理平台，接口是平台对外提供的 API 能力；文档里的「开放平台 API」该怎么理解？是不是还有一个叫「开放平台」的系统？

**答复**：

| 问题 | 说明 |
|------|------|
| 有没有两个平台？ | **没有。** 业务上只有 **网络安全漏洞管理平台** 这一套系统。 |
| 「开放平台 API」是什么？ | 该平台 **对外部第三方开放集成** 的 API 能力集合的 **产品/文档简称**，与 **「对外集成 API」「Partner API」** 同义。 |
| 为什么叫「开放平台」？ | 工程命名习惯：表示「对 Partner 开放接口」；**不是**对外再宣传一个独立品牌叫「开放平台」。 |
| 技术落点在哪？ | Partner 调用的业务接口统一在 **`/api/open/v1`**；Token 换取、控制台登录等不在该路径下（§2.2、§3.1.1）。 |
| 对外怎么说更准确？ | 优先：**「网络安全漏洞管理平台对外 API」**；引用本文档时可保留标题中的「开放平台 API」简称。 |

**一句话**：**平台 = 网络安全漏洞管理平台；开放平台 API = 该平台提供给外部的 API 能力，不是第二个平台。**
...
| 版本      | 日期       | 说明                                                                                                                                                                                                                                                                                                                                 |
| --------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **1.0.3** | 2026-05-19 | **§1.0** 拆分平台定位与命名（§1.0.1 平台本体、§1.0.2 澄清「开放平台 API」= 对外集成 API 能力简称，非独立平台）；**§1.3** 重写实例生命周期写接口对照表（3 类 6 路径；含任务创建共 8 路径）；**§2.1/§2.2** 补充 `clientId`/`clientSecret` 与认证服务 Base URL；**§3.1.1** 新增 OAuth / 简化 JSON Token 换取说明与示例；**§3.3** 明确 Webhook 验签与 REST 无关并指向 §6.0；**§4.2** 澄清创建任务幂等（`extTaskId` 必填、`Idempotency-Key` 可选及优先判重规则）；**§5.0.1** 统一表格列名体例；**§5.1.1** 示例后补充幂等说明；**§6** 补充 Webhook 触发说明，**§6.0** 迁入 Partner 接收端验签步骤；**附录 J** 新增接入 FAQ（J-0～J-6） |
