...
| 版本 | 日期 | 说明 |
|------|------|------|
| **1.0.1** | 2026-05-19 | §5.1.1 / §5.1.2 创建任务路径拆分：`POST /tasks/file`（XML）、`POST /tasks/vul`（JSON）；`file` 根元素改为 `<scanTask>`；XML 新增 `<task>`（`taskName` / `priority` / `callbackUrl`）；参数与 XML 元素严格 camelCase；`liveProbe.tcpPorts` 缺省 `1-65535` |
