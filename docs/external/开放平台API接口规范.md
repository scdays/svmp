...
| scanTemplateId | int | ○ | 平台扫描模板 ID（[附录 H.1](#h1-扫描模板-scantemplateid)）；**缺省 `0`** 按 `type` 自动匹配；所选模板**须支持**存活探测、端口扫描、标准漏洞排查三项 |
...
**§5.1.2 匹配规则（`scanTemplateId`）**

平台 scanTemplate **须同时支持**下列三项扫描能力（字段见 [附录 H.1](#h1-扫描模板-scantemplateid)）：

| 能力项 | H.1 字段 | 说明 |
|--------|----------|------|
| 存活探测 | `liveProbe` | ICMP/TCP 等主机存活探测 |
| 端口扫描 | `portScan` | 端口与服务识别 |
| 标准漏洞排查 | `vulnScan` | 系统/Web 漏洞扫描（`type=2` 时作用于 Web 目标） |

| scanTemplateId | 行为 |
|----------------|------|
| **0**（缺省） | 按 `type` 选用平台默认模板；默认模板**须启用**上表三项 |
| **>0** | 按 ID 加载；模板**须声明并支持**存活探测、端口扫描、标准漏洞排查；且 `applicableTypes` **须包含**当前 `type` |
| 不匹配 | 任一项能力缺失，或 `type` 不在 `applicableTypes` → **40004** |

预置模板示例见 [附录 H.1 · 平台预置示例](#h1-扫描模板-scantemplateid--server-内联扫描阶段)。
...
非法 `type` 返回 **40004**。`scanTemplateId` 对应模板未同时支持存活探测、端口扫描、标准漏洞排查，或 `type` 不在模板 `applicableTypes` 内 → **40004**。
...
|----|------|
| 根元素 | **`<scanTask>`** |
| 命名 | 所有 XML 元素与 JSON 字段名 **严格 camelCase**（如 `jumpHosts`、`typeField`、`vulId`） |
...
**内联 `<server>` 扫描阶段**（与 H.1 同构，不再使用 `<scanTemplate>` 容器）
...
H.1 / H.2 定义**扫描策略**与**报告/外发**的数据结构。平台可预置模板（通过 ID 引用）；§5.1.1 亦可在 `file` 内 **内联填写**，**不依赖平台内置模板**。
...
定义 **存活探测 → 端口扫描 → 漏洞扫描 / 口令猜测** 各阶段。内联时作为 `<server>` 的直接子元素（**不含** `<scanTemplate>` 容器与 ID 字段）。
...
| vulnScan.enabled · depth · expVerify | | 漏洞/Web 扫描 |
| pwdGuess.enabled · threadNum · timeoutSec · services[] | | 口令猜测（type **3**；与 `vulnScan` 可并存，执行策略见预置模板） |

**§5.1.2 匹配规则**

平台 scanTemplate（含 Partner 在 §5.1.1 内联于 `<server>` 的等效配置）**须同时支持**下列三项能力：

| 能力项 | H.1 字段 | 说明 |
|--------|----------|------|
| **存活探测** | `liveProbe` | `@enabled=true` 且配置 ICMP/TCP 等探测项 |
| **端口扫描** | `portScan` | `@enabled=true` 且配置端口策略 |
| **标准漏洞排查** | `vulnScan` | `@enabled=true` 且配置漏洞/Web 扫描参数 |

| scanTemplateId | 行为 |
|----------------|------|
| **0**（缺省） | 按 `type` 选用平台默认模板；默认模板**须启用**上表三项 |
| **>0** | 按 ID 加载；模板定义**须包含并支持**存活探测、端口扫描、标准漏洞排查；`applicableTypes` **须包含**当前 `type` |
| 校验失败 | 任一项能力缺失或未启用（按模板策略）→ **40001** / **40004**（见 §9） |

**平台预置示例**

| scanTemplateId | templateName | applicableTypes | 存活探测 | 端口扫描 | 标准漏洞排查 | 摘要 |
|----------------|--------------|-----------------|:--------:|:--------:|:------------:|------|
| **1001** | 标准漏洞排查 | [1] | ✓ | ✓ | ✓ | type=1 默认；三项全开 |
| **1002** | WEB 应用扫描 | [2] | ✓ | ✓ | ✓ | Web 深度扫描（`vulnScan.depth=4`） |
| **1003** | 主机口令猜测 | [3] | ✓ | ✓ | ✓ | 三项均声明；执行以 `pwdGuess` 为主，`vulnScan` 按模板策略 |

### H.2 报告模板 `reportTemplateId` / `<report>`

定义任务结束 **外发数据包** 的序列化格式与字段剖面（§5.6 / §7）。内联时根元素为 **`<report>`**（原 `<reportTemplate>`）。
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **1.0.1** | 2026-05-19 | §5.1.1 / §5.1.2 创建任务路径拆分：`POST /tasks/file`（XML）、`POST /tasks/vul`（JSON）；`scanTemplate` 须同时支持存活探测、端口扫描、标准漏洞排查三项 |
