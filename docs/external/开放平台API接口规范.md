...
| 项 | 说明 |
|----|------|
| **平台** | **网络安全漏洞管理平台** — 漏洞发现、验证、处置、修复核验与结果管理的统一业务系统 |
| **本文档** | 上述平台向 **外部第三方（Partner）** 提供的 **API 接口**说明（Base Path `/api/open/v1`） |
| **与扫描器** | Partner **不直连**扫描器；排查、复扫等由平台内部扫描执行层调度（见 §1.1） |
...
| | ③ 修复核验 | `POST /instances/{vulInfoID}/verify-fix` | 1 | 异步复扫；平台内部默认选用该漏洞实例**最近一次排查/扫描使用的扫描器**执行复扫，完成后更新 `vulInfoStat` 终态；Partner 可 Webhook 或轮询 |
| | | `POST /instances/verify-fix:batch` | 1 | 批量；复扫与扫描器选择规则同单条 |
...
| # | 疑问摘要 | 答复要点 | 详见 |
|---|----------|----------|------|
| J-0 | 对接的是扫描器吗？还是哪个系统？ | 对接 **网络安全漏洞管理平台**；Partner **不直连**扫描器，扫描/复扫由平台内部调度 | [§1.0](#10-平台说明) |
| J-6 | 旧资料里的「开放平台 API」如何理解？ | **历史/工程旧称**，指本平台 Partner 集成 API（`/api/open/v1`）；**不是**独立系统；现行文档名见 [§1.0](#10-平台说明) | 本节 J-6 |
...
**疑问**：需要对接扫描器吗？还是对接哪个系统？

**答复**：

1. **对接对象**：**网络安全漏洞管理平台**（漏洞全生命周期管理：排查/扫描、验证、处置、修复核验、结果外发等）。
2. **与扫描器的关系**：Partner **不直连**扫描器或扫描器厂商 API；排查、修复核验复扫等由平台**内部扫描执行层**调度（§1.0、§1.1）。
3. **本文档范围**：规范 Partner 调用 **`/api/open/v1`** REST 接口及接收 Webhook 的契约。

### J-6 · 「开放平台 API」名词如何理解（历史说明）

**疑问**：旧材料或工程讨论里出现「开放平台 API」，与本文档是什么关系？是不是还有一个叫「开放平台」的系统？

**答复**：

| 问题 | 说明 |
|------|------|
| 现行文档名称 | **《网络安全漏洞管理平台 · API 接口文档》**（即本文档） |
| 「开放平台 API」是什么 | **历史/工程旧称**，指上述平台向 Partner 提供的 **对外 API 能力**；Base Path 仍为 **`/api/open/v1`**（路径中 `open` 为模块标识） |
| 有没有两个平台？ | **没有。** 仅有 **网络安全漏洞管理平台**；「开放平台 API」**不是**第二个产品或独立系统名称 |
| 为何曾用此名 | 早期产品/工程命名习惯，强调「对第三方开放接口」；对外统一改用 **「网络安全漏洞管理平台 API」** |
| 现在应如何表述 | 对接 **网络安全漏洞管理平台 API**；技术实现仍调用 **`/api/open/v1`** |

**一句话**：**「开放平台 API」= 本平台对外 API 的旧称；现行文档与对外沟通统一使用「网络安全漏洞管理平台 · API 接口文档」。**
...
| 版本      | 日期       | 说明                                                                                                                                                                                                                                                                                                                                 |
| --------- | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **1.0.4** | 2026-05-19 | 文档更名为 **《网络安全漏洞管理平台 · API 接口文档》**；正文删除「开放平台」相关叙述，删除 §1.0.2；**§1.3.2** 修复核验补充复扫及默认扫描器选择规则；FAQ **J-0** 改为与「扫描器」对照，**J-6** 保留「开放平台 API」历史名词解释 |
| **1.0.3** | 2026-05-19 | **§1.0** 平台说明；**§1.3** 重写实例生命周期写接口对照表（3 类 6 路径；含任务创建共 8 路径）；**§2.1/§2.2** 补充 `clientId`/`clientSecret` 与认证服务 Base URL；**§3.1.1** 新增 OAuth / 简化 JSON Token 换取说明与示例；**§3.3** 明确 Webhook 验签与 REST 无关并指向 §6.0；**§4.2** 澄清创建任务幂等；**§5.0.1** 统一表格列名体例；**§6.0** Partner 接收端验签；**附录 J** 接入 FAQ |
