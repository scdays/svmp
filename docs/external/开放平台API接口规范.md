...
| 版本 | 日期 | 说明 |
|------|------|------|
| **1.0.2** | 2026-05-19 | XML：`<task>` → `<server>`；扫描目标迁至 `<server><targets>`（文本，`,`/`;` 分隔）；扫描阶段内联于 `<server>`（移除 `<scanTemplate>`）；`<reportTemplate>` → `<report>`；根级 `<targets>` 专用于登陆检查且始终存在 |
| **1.0.1** | 2026-05-19 | §5.1.1 / §5.1.2 创建任务路径拆分：`POST /tasks/file`（XML）、`POST /tasks/vul`（JSON）；`file` 根元素改为 `<scanTask>`；XML 新增任务信息元素（后演进为 `<server>`）；参数与 XML 元素严格 camelCase；`liveProbe.tcpPorts` 缺省 `1-65535` |
