...
```text
排查（POST /tasks 或 POST /tasks/file）
...
|----|-----|
| 能力 | `TASK_WRITE` |
| 说明 | 基于 **`file` XML 配置** 创建扫描任务；`type` **1 / 2 / 3** 均可。任务名称、优先级、回调 URL 写在 XML `<task>` 内；Partner 在 `file` 中填写目标，并 **二选一**：引用平台模板 ID，或内联 [附录 H](#附录-h--扫描模板与报告模板) 的 `<scanTemplate>` / `<reportTemplate>` |
...
| file | string | ✓ | 任务配置 XML（UTF-8）；根元素 `<ScanTask>`，须含 `<task>`，见 [附录 G](#附录-g--扫描任务配置文件-file) |
...
```text
1. POST /tasks/file 或 POST /tasks（§5.1.1 XML / §5.1.2 JSON；`extTaskId` 幂等）→ 保存 taskId
...
| 类型码 | 说明 | §5.1.1（`POST /tasks/file`） | §5.1.2（`POST /tasks`） |
|--------|------|------------------|----------------|
| **1** | 漏洞扫描 | ✓ | ✓ |
| **2** | WEB 应用扫描 | ✓ | ✓ |
| **3** | 口令猜测 | ✓ | ✓ |
...
| 项 | §5.1.1（`POST /tasks/file`） | §5.1.2（`POST /tasks`） |
|----|------------------|----------------|
...
| 任务信息 | **`<task>`** 必填，含 `taskName`、`priority`、可选 `callbackUrl` |
| 模板（二选一） | **A** · `scanTemplateId` + `reportTemplateId`（均 **>0**） · **B** · 内联 `<scanTemplate>` + `<reportTemplate>`（**不得**与 A 同时出现） |
...
| 路径 | 类型 | 必填 | 说明 |
|------|------|:---:|------|
| `/ScanTask/task/taskName` | string | ✓ | 任务名称 |
| `/ScanTask/task/priority` | enum | ○ | `LOW` / `MEDIUM` / `HIGH`；缺省 `MEDIUM` |
| `/ScanTask/task/callbackUrl` | string | ○ | 覆盖 Partner 默认 Webhook URL |
...
| 用法 | §5.1.1（`POST /tasks/file`） | §5.1.2（`POST /tasks`） |
|------|------------------|----------------|
