...
非法 `type` 返回 **40004**。`scanTemplateId` 对应模板未支持存活探测、端口扫描、漏洞扫描/WEB 应用扫描/口令猜测，或 `type` 不在模板 `applicableTypes` 内 → **40004**。
...
|----------------|------|
| **0**（缺省） | 按 `type` 选用平台默认模板；默认模板**须启用**上表三项 |
| **>0** | 按 ID 加载；模板定义**须包含并支持**存活探测、端口扫描、漏洞扫描/WEB 应用扫描/口令猜测；`applicableTypes` **须包含**当前 `type` |
