...
| format | string | `JSON` / `CSV` / `SVMP-TAR-GZ-XML` / `MIIT-2025-VULINFOLST` 等 |
...
#### `GET /exports/{exportId}/download` — 下载文件

| 项 | 值 |
|----|-----|
| 能力 | `EXPORT_READ` |

返回文件流；推荐 **`SVMP-TAR-GZ-XML`** 解压后含四阶段 XML（§7）。

#### `GET /tasks/{taskId}/exports` — 任务外发历史
