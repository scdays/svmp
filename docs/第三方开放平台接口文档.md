...
**扫描外发**：若验证阶段配置为触发引擎复扫 / POC 扫描，扫描完成后平台同样生成 `EXPORT_READY` 事件。此类外发 `exportStage=VERIFY_SCAN`，`dataType=SYSTEM_VULNERABILITY`，输出数据按 §5.8 的规范化结构表达，漏洞结果进入 `vulnerabilities[]`；`liveProbeResults[]`、`portScanResults[]` 仅在本次验证扫描实际产生对应结果时返回。
...
**扫描外发**：触发引擎模式下，修复核验扫描完成后除推进实例状态外，平台可生成 `EXPORT_READY` 事件。此类外发 `exportStage=VERIFY_FIX_SCAN`，`dataType=SYSTEM_VULNERABILITY`，输出数据按 §5.8 的规范化结构表达，漏洞结果进入 `vulnerabilities[]`；若核验扫描未发现原漏洞，可输出空 `vulnerabilities[]` 并在 `summary.totalInstances=0` 表示本次未发现系统漏洞实例。
...
| `VERIFY_SCAN` | 漏洞验证阶段触发复扫 / POC 扫描完成 | `SYSTEM_VULNERABILITY` | 输出系统漏洞数据；至少包含 `targets[]`，漏洞发现进入 `vulnerabilities[]` |
| `VERIFY_FIX_SCAN` | 修复核验阶段触发复扫完成 | `SYSTEM_VULNERABILITY` | 输出系统漏洞数据；若未发现漏洞，`vulnerabilities[]` 可为空并用 `summary.totalInstances=0` 表示 |

`VERIFY_SCAN` / `VERIFY_FIX_SCAN` 产生的外发与任务结束外发使用同一下载接口和同一 `xml` / `json` 序列化规则；区别仅在 `export.exportStage`、`export.dataType` 以及可选的 `relatedInstance` 关联信息。
...
├── export                 # 外发记录元数据
├── task                   # 当前开放接口任务信息
├── relatedInstance        # 验证 / 修复核验扫描外发时的关联漏洞实例
├── summary                # 汇总统计
...
| 验证 / 修复核验扫描 | `relatedInstance` + `targets[]` + `vulnerabilities[]` | 属于系统漏洞数据外发，使用 `exportStage` 区分 `VERIFY_SCAN` / `VERIFY_FIX_SCAN` |
...
#### 5.8.4 输出参数（`export` / `task` / `relatedInstance` / `summary`）
...
| `taskExport.relatedInstance.vulInfoID` | `/TaskExport/relatedInstance/vulInfoID` | string | 条件 | `VERIFY_SCAN` / `VERIFY_FIX_SCAN` 时必填，表示关联漏洞实例 |
| `taskExport.relatedInstance.vulInfoStat` | `/TaskExport/relatedInstance/vulInfoStat` | int | ○ | 扫描触发前或外发生成时的实例状态 |
| `taskExport.relatedInstance.verifyFixJobId` | `/TaskExport/relatedInstance/verifyFixJobId` | string | ○ | 修复核验异步任务 ID |
| `taskExport.relatedInstance.sourceAction` | `/TaskExport/relatedInstance/sourceAction` | enum | ○ | `verify` / `verifyFix` |
...
| relatedVulInfoID | string | 验证 / 修复核验扫描外发关联漏洞实例 ID |
...
字段说明以 §5.8.4–§5.8.7 为准。平台内部可参考 `report_by_*.xml` 的逻辑结构进行归并，但对外契约只承诺本节定义的规范化字段。

**验证 / 修复核验扫描外发差异**：结构与上例一致，但 `export.exportStage` 分别为 `VERIFY_SCAN` / `VERIFY_FIX_SCAN`，`export.dataType` 固定为 `SYSTEM_VULNERABILITY`，并增加 `relatedInstance`：
...
```json
{
  "taskExport": {
    ...
    "relatedInstance": {
      "vulInfoID": "VI-20260518-0001",
      "vulInfoStat": 6,
      "verifyFixJobId": "VFJ-20260518-0001",
      "sourceAction": "verifyFix"
    },
    "vulnerabilities": []
  }
}
...
| 引擎 XML 样本参考 | [`export-templates/xml-schemas/README.md`](../export-templates/xml-schemas/README.md) |

| 版本 | 日期 | 说明 |
|------|------|------|
| V1.0 | 2026-05-18 | 首版：REST + Webhook + 任务结束 XML 外发 |
| V1.1 | 2026-05-18 | §5 与设计方案 §五 全量对齐（参数、状态约束、示例） |
| V1.2 | 2026-05-19 | §5.8 补充 XML / JSON 外发说明与 `report_by_*.xml` 样本参考 |
| V1.3 | 2026-05-19 | §5.8 改为开放接口规范化输出模型；`format` 取值统一为 `xml` / `json` |
| V1.4 | 2026-05-19 | §5.8 扩充当前版本任务能力：主机存活探测、端口扫描、漏洞扫描均作为正式输出集合 |
| V1.5 | 2026-05-19 | §5.3 / §5.7 / §5.8 补充验证扫描、修复核验扫描外发；阶段扫描外发数据类型为系统漏洞 |
