...
任务结束外发输出物由开放平台按当前接口领域模型重新组装，支持 **`xml`** 与 **`json`** 两种 `format`。四个 `report_by_*.xml` 仅用于参考其「任务 → 目标 → 发现 → 详情 / 弱口令 / 配置 / 附录」逻辑分层，不作为对外文件名、根节点或字段名。
...
#### 5.8.3 输出参数（`export` / `task` / `summary`）
...
#### 5.8.4 输出参数（`targets[]` / `liveProbeResults[]` / `portScanResults[]`）
...
#### 5.8.7 `GET /exports/{exportId}` — 外发元数据
...
#### 5.8.8 `GET /exports/{exportId}/download` — 下载文件
...
#### 5.8.9 `GET /tasks/{taskId}/exports` — 任务外发历史
...
字段说明以 §5.8.3–§5.8.6 为准。平台内部可参考 `report_by_*.xml` 的逻辑结构进行归并，但对外契约只承诺本节定义的规范化字段。
