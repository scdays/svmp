...
│   └── appendices         # 端口、Banner、系统、软件等表格化附录
...
| 参考结构 | 规范化落点 | 说明 |
|----------|------------|------|
| 主机/系统漏洞结果 | `targets[]` + `vulnerabilities[]` | 主机维度进入目标，漏洞实例和漏洞详情合并为开放接口实例字段 |
...
#### 5.8.4 输出参数（`targets[]` / `vulnerabilities[]`）
...
字段说明以 §5.8.3–§5.8.5 为准。平台内部可参考 `report_by_*.xml` 的逻辑结构进行归并，但对外契约只承诺本节定义的规范化字段。
