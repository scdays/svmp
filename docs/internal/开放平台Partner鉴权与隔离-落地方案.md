...
> ⚠️ **历史方案说明**：本文档中「认证/登录服务 = clover」「API 网关 = morningglory」的 Partner 改造路径 **已废弃**。当前采用 **零侵入** 架构：**partner-gateway** + **open-api-service**（Token 与 Partner 数据均在 open-api-service）。以 [组件职责与接口映射 §0.2](./组件职责与接口映射.md) 与 [开放平台API治理与调用生命周期](./开放平台API治理与调用生命周期.md) 为准。下文保留作设计参考。

> **适用前提（历史）**：Token 在**用户/认证登录服务**生成并写入缓存；**网关只校验、不签发**。  
