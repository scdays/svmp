...
> **关联**：[映射表](./Open%20API与vul-pass内部接口映射表.md) · [执行面落地方案](./开放平台对外REST执行面-分期落地方案.md) · [Mock 实例落库 PRD](./open-api-mock-instance-db-PRD.md)（OP-MOCK-P1-DB） · [**半人工联调配置指南**](./open-api-mock-manual-配置指南.md）
...
映射表 [§6 决策记录](./Open%20API与vul-pass内部接口映射表.md) 在 vul-pass 开发并行填写；**不阻塞** mock 联调。
...
| 1.1 | 2026-06-13 | 关联 OP-MOCK-P1-DB：任务 FINISHED 后实例写入 `open_vuln_instance`，见 [PRD](./open-api-mock-instance-db-PRD.md) |
