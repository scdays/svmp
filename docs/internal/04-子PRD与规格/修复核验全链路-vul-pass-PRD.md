...
| 项 | 定稿 |
|----|------|
| 步骤 | 任务预检 → 任务下发 → [阶段子任务链] → 结果回收 → 稽核完成 → 任务结束（Wave G；核验链=连通性检测→修复核验） |
| 展示 | 有 `lifecycleSteps` 时始终展示；后端未返回时前端按 `tskPhase` 兜底 |
...
### 17.2 状态机（v1.4.7 Wave H）

> 主/子任务状态拆分；读路径兼容旧码 0～8。主链路展示仍可由 `TaskLifecycleResolver` 推导。

#### 17.2.1 主任务状态（`vul_scan_task.tsk_stat`）

| code | 枚举 | 中文 | 说明 |
|------|------|------|------|
| 10 | PRECHECK | 任务预检 | 计划已生成、待确认下发 |
| 20 | PENDING_START | 待启动 | 已确认，等待启动（在线/离线通用，模式见 `tskModel`） |
| 30 | RUNNING | 执行中 | 子任务链进行中（链内细节见 `wave` / `currentStepCode`） |
| 40 | RECYCLING | 结果回收 | 子任务已齐，汇总/回收中 |
| 50 | AUDIT_DONE | 稽核完成 | 稽核通过 |
| 60 | FINISHED | 任务结束 | 终态成功 |
| 90 | FAILED | 失败 | 终态失败 |

#### 17.2.2 子任务状态（`vul_scan_task_sub.tsk_stat`）
